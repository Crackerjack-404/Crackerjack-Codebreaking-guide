...
As much as this challenge rewards speed, it also rewards accuracy. In the later challenges especially, I often found that acting to quickly meant committing to an idea before the problem had been properly understood. When an initial idea fails, it is easy to move on to another, and then another again, without ever fully engaging with the structure of the cipher itself.

A more effective approach might be to first pause. After all, there's a reason races start with \textit{On Your Mark... Get Set.. Go!} and not just someone shouting \textit{GO!!} The pause is part of the start. It gives you time to settle, look ahead, and be ready to move with some sense of direction rather than pure adrenaline. 
...
One of the most important questions yourself can ask early on is: \textbf{What is one unit of this ciphertext? }

In many familiar contexts, it is natural to assume that each character corresponds to a letter. In codebreaking, this assumption is not always correct.

A single symbol might represent a letter, or a pair of letters, or a number, or a code. Maybe the groupings of such symbols or characters only make sense when they appear in a certain pattern. 

For example, in Morse code, individual dots and dashes are not letters. Only when they are grouped in the correct way do they form meaningful units, so treating each symbol independently would not lead to a solution.

Recognising the correct unit is not always immediate. However, being aware that this question exists can prevent you from making unhelpful assumptions too early.
