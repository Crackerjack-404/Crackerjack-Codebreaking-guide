...
Fortunately for National Cipher Challenge, every single piece of information is intentional \footnote{And even if something isn't, that's a feature, not a bug}. The ciphers are designed with intention around a theme/story, which means that the plaintext is not going to be arbitrary. So in many cases, we can infer a lot of information about content in the ciphertexts from context on the site, especially the \texttt{Case Files} section. Other examples include: 
